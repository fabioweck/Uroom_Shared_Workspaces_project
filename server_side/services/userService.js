...
async function getOwnerContact(workspace_id) {
    const workspaces = await loadJsonFile('workspaces.json');
    const users = await loadJsonFile('users.json');

    return new Promise(async (resolve, reject) => {

        const workspaceMatch = workspaces.find(w => w.workspace_id == workspace_id);
        const userMatch = users.find(u => u.user_id == workspaceMatch.user_id);

        delete userMatch.password;
        delete userMatch.user_id;

        //  console.log('function getOwnerContact test', userMatch);

        resolve(userMatch);

    }).catch(err => {
        const code = err.statusCode || 500;
        const message = err.message || 'Error occurred while return owner contact';
        throw { code, message };
    });
};
// console.log
...
async function updateReservedDate(dates) {
    ...
    const workspace_id = dates[0].workspace_id;
    ...
    return new Promise(async (resolve, reject) => {
        ...
        const ownerContact = getOwnerContact(workspace_id)
...
module.exports = { addUser, login, addProperty, addWorkspace, getWorkspaceByOwner, delistWorkspace, delistProperty, getReservedDate, updateReservedDate, updateUser, updateProperty, updateWorkspace, getOwnerContact }